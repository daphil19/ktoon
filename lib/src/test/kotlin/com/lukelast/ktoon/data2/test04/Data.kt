    ...
    inventory = listOf(
        SportsCar(
            ...
            engineSpec = EngineSpec(
                ...
                displacement = 0.000001, // Tests: Canonical decimal format (no Scientific notation)
                horsepower = 0,
            ...
            // 2. ":" contains colon -> ":"
            // 3. "[" contains bracket -> "["
            // 4. " " contains space (trimmed by default, so usually quoted if intended as value)
            // 5. "-" starts with hyphen -> "-"
            features = listOf(",", ":", "[", " ", "-"),

            // Tests: Tabular Array with value containing active delimiter
            lapTimes = listOf(
                LapTime(track = "Track, A", seconds = 60.0), // "Track, A" must be quoted inside the row
